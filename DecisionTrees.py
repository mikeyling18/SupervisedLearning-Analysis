...
def create_graphs(train_size, minimum_leaves, X, Y, name):
    ...
    for min_leaf in minimum_leaves:
        ...
        for size in training_sizes:
            ...
            original_tree.fit(train_X, train_Y)
            train_time = time.time() - start_time

            # Get Predictions
            start_time = time.time()
            original_predicted_test_y = original_tree.predict(test_X)
            predict_time = time.time() - start_time

            original_predicted_train_y = original_tree.predict(train_X)
